
<div class="container-fluid mt-5 mb-5" style="background-color: #f4f1eb; padding:20px 5px">
<div class="row">
<div class="col-12 text-center d-flex justify-content-center align-items-center">
   <a href="index.html">Home</a><span class="px-3">/</span>
   <p class="margin-0">Profile</p>
</div>
</div>
</div>

<div class="container">
   <div class="row">
      <div class="col-lg-4 col-xl-4">
         <div class="card-box text-center">
            <img src="https://bootdey.com/img/Content/avatar/avatar7.png" class="rounded-circle avatar-xl img-thumbnail" alt="profile-image">
            <h4 class="mb-0">Demo</h4>
            <p class="text-muted">@Subtext</p>
            <a href="<?=base_url()?>/Home/user_edit_profile" class="txt-deco-no">
            <button type="button" class="btn btn-primary btn-xs waves-effect mb-2 waves-light white">Edit Profile</button>
            </a>
            <br>
            <button class="btn btn-block order-button"><a href="index.html" class="green txt-deco-no"><i class="bi bi-arrow-left px-1"></i>Continue Shopping</a></button>
            <div class="text-left mt-3">
               <h4 class="font-13 text-uppercase">About Me :</h4>
               <p class="text-muted font-13 mb-3">
                  Hi I'm Demo,has been the industry's standard dummy text ever since the
                  1500s, when an unknown printer took a galley of type.
               </p>
               <p class="text-muted mb-2 font-13"><strong>Full Name :</strong> <span class="ml-2">Demo</span></p>
               <p class="text-muted mb-2 font-13"><strong>Mobile :</strong><span class="ml-2">(123)
                  123 1234</span>
               </p>
               <p class="text-muted mb-2 font-13"><strong>Email :</strong> <span class="ml-2 ">user@email.domain</span></p>
               <p class="text-muted mb-1 font-13"><strong>Location :</strong> <span class="ml-2">india</span></p>
            </div>
         </div>
         <!-- end card-box -->
      </div>
      <!-- end col-->
      <div class="col-lg-8 col-xl-8">
         <div class="card-box">
            <div class="tab-content" style="padding: 10px 10px ;">
               <h5 class="mb-3 text-uppercase"><i class="mdi mdi-cards-variant mr-1"></i>
                  Product Detail
               </h5>
               <div class="table-responsive">
                  <table class="table table-borderless mb-0">
                     <thead class="thead-light">
                        <tr>
                           <th>#</th>
                           <th>Product Name</th>
                           <th>Order Date</th>
                           <th>Recive Date</th>
                           <th>Amount</th>
                        </tr>
                     </thead>
                     <tbody>
                        <tr>
                           <td>1</td>
                           <td>Lorem ipsum dolor sit amet</td>
                           <td>01/01/2015</td>
                           <td>10/15/2018</td>
                           <td>₹1,796.00</td>
                        </tr>
                        <tr>
                           <td>2</td>
                           <td>Lorem ipsum dolor sit amet</td>
                           <td>21/07/2016</td>
                           <td>12/05/2018</td>
                           <td>₹1,796.00</td>
                        </tr>
                        <tr>
                           <td>3</td>
                           <td>Lorem ipsum dolor sit amet</td>
                           <td>18/03/2018</td>
                           <td>28/09/2018</td>
                           <td>₹1,796.00</td>
                        </tr>
                        <tr>
                           <td>4</td>
                           <td>Lorem ipsum dolor sit amet</td>
                           <td>02/10/2017</td>
                           <td>07/05/2018</td>
                           <td>₹1,796.00</td>
                        </tr>
                        <tr>
                           <td>5</td>
                           <td>Lorem ipsum dolor sit amet</td>
                           <td>17/01/2017</td>
                           <td>25/05/2021</td>
                           <td>₹1,796.00</td>
                        </tr>
                     </tbody>
                  </table>
               </div>
            </div>
            <!-- end timeline content-->
            <!-- end settings content-->
         </div>
         <!-- end tab-content -->
      </div>
      <!-- end card-box-->
   </div>
   <!-- end col -->
</div>
</div>
